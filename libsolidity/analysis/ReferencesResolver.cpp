...
bool ReferencesResolver::visit(InlineAssembly const& _inlineAssembly)
{
	...
	m_yulState.emplace(_inlineAssembly.annotation());
...
{
	m_yulState.value().m_insideFunction = true;
	this->operator()(_function.body);
	m_yulState.value().m_insideFunction = false;
}

void ReferencesResolver::operator()(yul::Block const& _block)
{
	auto previousScope = m_yulState.value().m_scope;
	walkVector(_block.statements);
	m_yulState.value().m_scope = previousScope;
}

void ReferencesResolver::operator()(yul::Identifier const& _identifier)
{
	if (m_yulState.value().m_scope.find(_identifier.name) != m_yulState.value().m_scope.end())
		return;
	...
	if (auto var = dynamic_cast<VariableDeclaration const*>(declarations.front()))
		if (var->isLocalVariable() && m_yulState.value().m_insideFunction)
		{
			declarationError(_identifier.location, "Cannot access local Solidity variables from inside an inline assembly function.");
			return;
		}

	m_yulState.value().m_annotation.externalReferences[&_identifier].isSlot = isSlot;
	m_yulState.value().m_annotation.externalReferences[&_identifier].isOffset = isOffset;
	m_yulState.value().m_annotation.externalReferences[&_identifier].declaration = declarations.front();
}
...
void ReferencesResolver::operator()(yul::VariableDeclaration const& _varDecl)
{
	for (auto const& identifier: _varDecl.variables)
	{
		...
		m_yulState.value().m_scope.emplace(identifier.name);
	}
